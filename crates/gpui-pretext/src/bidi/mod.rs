...
thread_local! {
    /// Reusable scratch buffer for the character list during bidi level computation.
    static BIDI_CHARS_SCRATCH: RefCell<Vec<char>> = const { RefCell::new(Vec::new()) };
...
fn compute_bidi_levels(s: &str) -> Option<Vec<i8>> {
    ...
    BIDI_CHARS_SCRATCH.with(|chars_scratch| {
        let mut chars = chars_scratch.borrow_mut();
        chars.clear();
...
pub fn compute_segment_levels(normalized: &str, seg_starts: &[usize]) -> Option<Vec<i8>> {
    let bidi_levels = BIDI_LEVELS_CACHE
        .with(|cache| {
...
#[cfg(test)]
fn bidi_chars_scratch_capacity() -> usize {
    BIDI_CHARS_SCRATCH.with(|s| s.borrow().capacity())
}
