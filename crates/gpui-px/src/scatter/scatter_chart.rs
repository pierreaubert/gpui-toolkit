...
use super::types::ScatterDataCache;
...
/// `cached_scatter_points` call is a pointer-identity cache hit.
fn refill_scatter_points_if_reusable(
    x: &Arc<[f64]>,
    y: &Arc<[f64]>,
    cache: &mut ScatterDataCache,
...
mod streaming_cache_tests {
    ...
    #[test]
    fn mapped_scatter_points_reuse_unique_storage_and_preserve_held_frames() {
        ...
        let held_first = cached_scatter_points(&x1, &y1, &mut cache);
        let second = cached_scatter_points(&x2, &y2, &mut cache);
        ...
        drop(second);
        let third = cached_scatter_points(&x3, &y3, &mut cache);
        assert_eq!(Arc::as_ptr(&third), reusable_ptr);
        assert_eq!((third[1].x, third[1].y), (9.0, 11.0));
    }

    #[test]
    fn shared_scatter_replacement_validates_and_prepares_primary_data() {
        let mut chart = crate::scatter(&[0.0], &[1.0]);
        chart
    ...
    #[test]
    fn invalid_shared_scatter_replacement_does_not_poison_current_data() {
        let original_x: Arc<[f64]> = Arc::from([0.0, 1.0]);
        let original_y: Arc<[f64]> = Arc::from([2.0, 3.0]);
        let mut chart = crate::scatter(&original_x, &original_y);
        assert_eq!(chart.prepare_primary_data(), 2);

        assert!(matches!(
            chart.replace_primary_data_shared(
                Arc::from([4.0, 5.0]),
                Arc::from([6.0, f64::INFINITY]),
            ),
            Err(ChartError::InvalidData { field: "y", .. })
        ));
        assert_eq!(chart.x.as_ref(), original_x.as_ref());
        assert_eq!(chart.y.as_ref(), original_y.as_ref());
        assert_eq!(chart.prepare_primary_data(), 2);

        let mut empty_chart = crate::scatter(&[], &[]);
...
impl ScatterChart {
    ...
    pub fn replace_primary_data_shared(
        ...
        if !refill_scatter_points_if_reusable(&x, &y, &mut self.primary_data_cache)? {
    ...
    pub fn prepare_primary_data(&mut self) -> usize {
        cached_scatter_points(&self.x, &self.y, &mut self.primary_data_cache).len()
    }
