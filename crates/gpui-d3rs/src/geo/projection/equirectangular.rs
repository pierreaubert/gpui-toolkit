...
impl Projection for Equirectangular {
    ...
    fn project_rotated(&self, lambda: f64, phi: f64) -> (f64, f64) {
        let (cx, cy) = (radians(self.config.center.0), radians(self.config.center.1));

        (
            self.config.translate.0 + self.config.scale * (lambda - cx),
            self.config.translate.1 - self.config.scale * (phi - cy),
        )
    }

    fn invert(&self, x: f64, y: f64) -> Option<(f64, f64)> {
        let (cx, cy) = (radians(self.config.center.0), radians(self.config.center.1));
        let lambda = (x - self.config.translate.0) / self.config.scale + cx;
        let phi = -(y - self.config.translate.1) / self.config.scale + cy;
    ...
    fn longitude_unwrap_center(&self) -> Option<f64> {
        // See Mercator::longitude_unwrap_center for the rationale: the
        // antimeridian seam is determined by rotation, not by the planar center.
