//! Keychain storage for credentials used by an AU extension.
//!
...
pub fn write_credentials(url: &str, username: &str, password: &[u8]) -> Result<()> {
    ...
    unsafe {
        ...
        let operation = if status == ERR_SEC_ITEM_NOT_FOUND {
        ...
        anyhow::ensure!(
            ...
            "{operation} AU keychain item failed: {status}"
        );
        Ok(())
    }
}
...
pub fn read_credentials(url: &str) -> Result<Option<(String, Vec<u8>)>> {
    ...
    unsafe {
        ...
        match status {
            ...
            _ => anyhow::bail!("reading AU keychain item failed: {status}"),
        }
        ...
        let password = CFType::wrap_under_get_rule(*password)
            .downcast::<CFData>()
            .context("password was not data in AU keychain item")?;
...
pub fn delete_credentials(url: &str) -> Result<()> {
    ...
    unsafe {
        ...
        anyhow::ensure!(
            ...
            "deleting AU keychain item failed: {status}"
        );
        Ok(())
    }
}
