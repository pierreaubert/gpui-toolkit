...
- `conflict.rs` — `detect_conflicts()`: finds duplicate documented key specs after GPUI parsing; executable contexts are not available, returns `Vec<KeyConflict>`
...
- Conflict detection checks duplicate documented key specs across all registered providers; callers must evaluate executable GPUI contexts separately
